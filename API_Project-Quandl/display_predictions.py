...
class PDFDataExtractor:
    ...
    def extract_data(self) -> List[Dict[str, Optional[str]]]:
        ...
        """
        results = []
        all_text_content = []  # Store all text for cross-page validation
        
        with pdfplumber.open(self.pdf_path) as pdf:
            # First pass: collect all text content
            for page in pdf.pages:
                all_text_content.append(page.extract_text() or "")
            
            # Second pass: extract data
            for page_num, page in enumerate(pdf.pages):
                # Get character-level data for better OCR analysis
                ...
                # Strategy 2: Try positioned text with character analysis
                positioned_data = self._extract_from_positioned_text_with_chars(page, chars)
                if positioned_data:
        ...
        # Post-process: verify using all available context
        results = self._verify_and_clean_results(results, all_text_content)
        
        return results
    
    def _extract_from_positioned_text_with_chars(self, page, chars) -> List[Dict[str, Optional[str]]]:
        """Extract data using character-level positioning for better accuracy."""
        if not chars:
            # Fallback to word-based extraction
            return self._extract_from_positioned_text(page)
        ...
        # Sort characters in each line by x-coordinate
        for y in lines:
            lines[y].sort(key=lambda c: c.get('x0', 0))
        
        # Reconstruct text from characters with better accuracy
        results = []
        sorted_lines = sorted(lines.items(), key=lambda x: x[0])
        
        for i, (y, chars_in_line) in enumerate(sorted_lines):
            line_text = ''.join([c.get('text', '') for c in chars_in_line])
            
            # Look for transaction data pattern
            if 'transaction' in line_text.lower() and 'date' in line_text.lower():
                # This is likely a header, next lines are data
                for j in range(i+1, min(i+10, len(sorted_lines))):
                    _, data_chars = sorted_lines[j]
                    data_line = ''.join([c.get('text', '') for c in data_chars])
                    
                    if not data_line.strip():
                        continue
                    
                    record = self._extract_structured_row(data_line, line_text)
                    if any(record.values()):
                        results.append(record)
        
        return results
    
    def _verify_and_clean_results(self, results: List[Dict], all_text: List[str]) -> List[Dict]:
        """Verify extracted data and fix common OCR errors using context."""
        cleaned_results = []
        
        # Join all text for context analysis
        full_text = '\n'.join(all_text)
        ...
        for record in results:
            ...
            if record.get('date'):
                ...
                if self.use_ocr_correction:
                    # Look for date context in surrounding text
                    # For example, if document says "June 27, 2025" elsewhere
                    # we can verify the numeric date
                    ...
                    # If we find "June 27, 2025" in text, we know month should be 06
                    # But if date extracted is 05/27/2025, there's a discrepancy
                    date_parts = date_str.split('/')
                    if len(date_parts) == 3:
                        month, day, year = date_parts
                        
                        # Look for month name + day + year pattern
                        for month_name, month_num in month_names.items():
                            pattern = rf'\b{month_name}\s+{int(day)},?\s+{year}\b'
                            if re.search(pattern, full_text, re.IGNORECASE):
                                # Found matching month name, verify it matches
                                if month.zfill(2) != month_num:
                                    print(f"Date correction: {month}/{day}/{year} -> {month_num}/{day}/{year} based on '{month_name}' in text")
                                    record['date'] = f"{month_num}/{day}/{year}"
                                break
